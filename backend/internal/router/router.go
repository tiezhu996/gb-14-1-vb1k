...
func New(deps Deps) *gin.Engine {
	...
	rejudgeRepo := repository.NewRejudgeRepository(db)
	...
	rejudgeService := service.NewRejudgeService(rejudgeRepo, submissionRepo, problemRepo, userRepo, statRepo, judgeService, deps.Logger)
	...
	rejudgeHandler := handler.NewRejudgeHandler(rejudgeService)
	...
	api := r.Group("/api/v1")
	{
		...
		registerRejudgeRoutes(api, rejudgeHandler, auth, admin, audit)
